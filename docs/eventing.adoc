...
* For _Developers_ a more opinionated approach is taken, shielding the developer from direct resource management. There are currently three different ways how a service can be registered for events:
+
** Direct linkage to an importer
** Subscription to a channel
** Subscription to a broker with a trigger
+
All of these different ways are to be shielded behind a common UX. In <<connections>> a suggestion is made how this can be achieved with a noun "connection" that fits into the general '<noun> <verb>' scheme of `ln` and is not directly connected to any backing CR. For <<sequences>> a workflow is suggested to build up such a sequence of connected transforming services.

To summarize this summary :) there are three key take-aways from this document:

[IMPORTANT]
====
. Differentiation between the _Integrator_ and _Developer_ role for Knative eventing (though both can be played by the same user of course).
...
NOTE: As an _Integrator_ I want to manage importers (sources) and the infrastructure elements like brokers and channels to set up the eventing topology.
...
I want to find out all importer types available for creating a new importer
...
I want to create a new importer
...
I want to list all importers
I want to see details of an importer
Type
Configuration options
Usages (broker, sinks)
I want to delete an importer

[[brokers]]
==== Brokers

I want to create a broker in a namespace
I want to list all brokers in a namespace
I want to see the details of a broker
What channels are attached
Importers referencing the broker
I want to remove a broker
...
NOTE: As an _Integrator_ I want to find all channel types which are available by a given Knative installation
...
NOTE: As an _Integrator_ I want to create a channel with a specified type
...
===== List all channels
NOTE: As an _Integrator_ and as a _Developer_ I want to list all channels

===== Show channel details
NOTE: As an _Integrator_ and as a _Developer_ I want to see the details of a channel

Triggers attached to the channel
Broker which uses the channel

===== Remove a channel
NOTE: As an _Integrator_ I want to remove a channel
...
NOTE: As a _Developer_ I want to use the eventing topology to receive events for which I can register my services with filtering and chaining.
...
NOTE: As a _Developer_ I want to connect a service to the eventing infrastructure.
...
NOTE: As a _Developer_ I want to update a connection
...
NOTE: As a _Developer_ I want to see the details of a connection
...
NOTE: As a _Developer_ I want to list all connections
...
NOTE: As a _Developer_ I want to delete a connection.
...
I want to interactively build up a Sequence (new in Eventing 0.7.0) by subsequent calls to an “append” or “insert” calls for adding transformer services.
I want to list all sequences
I want to remove a sequence (with usage check)
I want to see the details of a sequence
