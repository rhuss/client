...
[quote]
____
As am _Integrator_ I want to find all channel types which are available by a given Knative installation
____
...
[NOTE]
====
If channel creation is not exposed directly, but done indirectly e.g. when creating or modifying a broker, then this sub-command should be moved to this location as well.
====
...
[NOTE]
====
Whether a channel can be directly created or is only created as part of `broker` management should be discussed
====
