...
Before diving in the detailed use case let's revisit the roles and create two use case epics for each role before diving into more specialieded use cases:
...
The developer is the user of the eventing topololgy.
...
In the following and depend on which side of the <<abstraction,spectrum of abstractions>> we are moving, the full set of CRUD verbs are supported when managing simple resources:
...
[source]
----
# Connect a service directly to an importer.
$ kn service connect myservice --importer k8sapievents

# Alternative syntax:
$ kn service connect myservice --target importer:k8sapivevents
...
[source]
----
$ kn service update-connection myservice-001 --filter <new filter>
----

[quote]
____
As a _Developer_ I want to get the details of a connection
____

[source]
----
$ kn service show-connection myservice-001
...
$ kn service delete-connection myservice-001
----

[NOTE]
====
Instead of piggy pack on the `service` command group a dedicated `connection` command group could be implemented as alternative, leading to commands like `kn connection create <name> --service myservice --broker mybroker`. The same might apply for the other subscription use cases, too. The good thing would be, that the connection name could be directly specified instead of adding it with a --name option as in the example above.

The suggested commands would then translate to the standard <<management-operations, management operations>>:

* `kn connection create myconn01 --service myservice --importer myimporter`
* `kn connection update myconn02 --filter=<new filter>`
* `kn connection show myconn02`
* `kn connection list`
* `kn connection delete myconn01`

Of course also a mixed form could be offered: CRUD for `connection` (maybe minus "create") + a `kn service connect` for connecting to a service.
====
...
See <<custom-types>> for more details how detection and channel plugin work.

==== Create a new channel

[NOTE]
====
Whether a channel can be directly created or is only created as part of `broker` management should be discussed. It makes sense to create channels directly for a direct
====

[quote]
____
I want to create a channel from the CLI
____
